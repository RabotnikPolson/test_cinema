    ...
    datasets: [
      {
        ...
        backgroundColor: ["#C9A84C", "#1A1A2A"],
        hoverBackgroundColor: ["#D4B150", "#2A2A3A"],
        borderColor: ["#07070F", "#07070F"],
        borderWidth: 2,
      },
    ],
  };
    ...
    <div className="ratio" style={{ flexDirection: "column" }}>
      <div style={{ width: 180, height: 180, position: "relative" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
          <span style={{ fontFamily: "Bebas Neue", fontSize: "28px", color: "#D4D8E8" }}>{domesticPercent.toFixed(1)}%</span>
      ...
      <div className="ratio__legend" style={{ marginTop: "1rem", width: "100%", display: "grid", gap: "0.5rem" }}>
        <div className="ratio__legend-row" style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "#9A9EB8" }}>
          ...
          Казахстанское
          <b style={{ marginLeft: "auto", color: "#D4D8E8", fontFamily: "JetBrains Mono" }}>{Math.round(ratio.domesticWatchSeconds / 3600)} ч</b>
        </div>
        <div className="ratio__legend-row" style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "#9A9EB8" }}>
          <span className="ratio__dot" style={{ background: "#1A1A2A", border: "1px solid #2A2A3A", width: 10, height: 10, borderRadius: "50%" }} />
          Зарубежное
          <b style={{ marginLeft: "auto", color: "#D4D8E8", fontFamily: "JetBrains Mono" }}>{Math.round(ratio.foreignWatchSeconds / 3600)} ч</b>
        </div>
      </div>
    </div>
  );
...
export default function AdminAnalyticsPage() {
  ...
  const aiHealthQuery = useQuery({
    queryKey: ["ai-health"],
    queryFn: async () => {
      const response = await aiHttp.get("/health");
  ...
  const triggerRetrain = useMutation({
    mutationFn: () => aiHttp.post("/api/v1/ml/retrain"),
  ...
  return (
    <div className="analytics-dashboard">
      ...
      <section className="panel" style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
        <div className="panel__head">
          <div>
            <h3 className="panel__title">Активность пользователей</h3>
            <span className="panel__sub">userActivity · текущая неделя</span>
          </div>
        </div>
        <UserActivityLineChart activityData={data.userActivity} />
      </section>
