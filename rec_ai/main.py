...
async def recommend_collaborative_item(movie_id: int, limit: int = 5):
    recs = recommender.get_collaborative_users_also_watched(movie_id, ML_MODEL["df"], limit)
...
async def frontend_tab_recommendations(type: str, movie_id: int, limit: int = 15, user_id: Optional[int] = None):
    ...
    try:
        result = {
            ...
            "collaborative-item": lambda: recommender.get_collaborative_users_also_watched(movie_id, ML_MODEL["df"], limit),
        }.get(type)
        if result is None:
